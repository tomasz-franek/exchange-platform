...
type ReportState = {
  ...
  pairPeriodResponse: PairPeriodResponse | null;
  errorMessageList: ErrorMessage[];
  isLoading: boolean;
};
export const initialReportState: ReportState = {
  pairPeriodResponse: null,
  accountsReportResponse: [],
  errorMessageList: [],
  isLoading: false,
};

export interface ReportPairRequest {
  pair: Pair;
  period: number;
}
...
export const ReportStore = signalStore(
  ...
  withMethods(
    (
      ...
      loadPairPeriodReport: rxMethod<ReportPairRequest>(
        pipe(
          tap(() => patchState(store, { isLoading: true })),
          switchMap((reportPairRequest) => {
            return apiService
              .loadPairPeriodReport(
                reportPairRequest.pair,
                reportPairRequest.period,
              )
              .pipe(
                tapResponse({
                  next: (pairPeriodResponse) =>
                    patchState(store, {
                  ...
                  error: (errorResponse: HttpErrorResponse) => {
                    ...
                    patchState(store, {
                      pairPeriodResponse: {} as PairPeriodResponse,
                    });
                  },
                  finalize: () => patchState(store, { isLoading: false }),
                }),
