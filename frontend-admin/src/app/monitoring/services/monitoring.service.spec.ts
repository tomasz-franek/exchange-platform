...
describe('MonitoringService', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        {provide: ActuatorExternalService, useValue: actuatorExternalServiceSpy},
        {provide: ActuatorInternalService, useValue: actuatorInternalServiceSpy}
      ],
    });
  ...
  it('should load Admin Service Health Check', () => {
    ...
    actuatorAdminService.loadActuatorAdminHealthCheck.and.returnValue(of(healthCheck) as any);

    apiService.loadActuatorAdminHealthCheck().subscribe((operations) => {
      expect(operations).toEqual(healthCheck);
    });

    expect(actuatorAdminService.loadActuatorAdminHealthCheck).toHaveBeenCalled();
  });
  ...
  it('should load External Service Health Check', () => {
    ...
    actuatorExternalService.loadActuatorExternalHealthCheck.and.returnValue(of(healthCheck) as any);

    apiService.loadActuatorExternalHealthCheck().subscribe((operations) => {
      expect(operations).toEqual(healthCheck);
    });

    expect(actuatorExternalService.loadActuatorExternalHealthCheck).toHaveBeenCalled();
  });
  ...
  it('should load Internal Service Health Check', () => {
    ...
    actuatorInternalService.loadActuatorInternalHealthCheck.and.returnValue(of(healthCheck) as any);

    apiService.loadActuatorInternalHealthCheck().subscribe((operations) => {
      expect(operations).toEqual(healthCheck);
    });

    expect(actuatorInternalService.loadActuatorInternalHealthCheck).toHaveBeenCalled();
  });
